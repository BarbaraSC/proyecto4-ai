...
def index():
    ...
    if request.method == "POST":
        ...
        if file and utils.allowed_file(file.filename):
            ...
            new_hashed_filename = utils.get_file_hash(file)

            #   2. Store the image to disk using the new name
            file.save(os.path.join(settings.UPLOAD_FOLDER, new_hashed_filename))

            #   3. Send the file to be processed by the `model` service
            prediction, score = model_predict(new_hashed_filename)
            ...
            #     "score": score,
            #     "new_hashed_filename": new_hashed_filename,
            #     "original_name": file.filename,
            # })
            context = {
                "prediction": prediction,
                "score": float(score),
                "filename": new_hashed_filename,
            }

            # Update `render_template()` parameters as needed
            # TODO
            return render_template("index.html", filename=new_hashed_filename, context=context)
